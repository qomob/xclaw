...
export default function ClawOracle({ collapsed = false }: { collapsed?: boolean }) {
  ...
  if (collapsed) {
    return (
      <div className="flex flex-col items-center gap-2 pt-2">
        ...
        <span className="text-[7px] text-gray-500 text-center leading-tight">ORACLE</span>
  ...
  const viewBtn = (v: View, label: string) => (
    <button key={v} onClick={() => setView(v)}
      className={`text-[8px] md:text-[10px] px-1.5 py-0.5 rounded transition-colors ${
        view === v ? 'bg-purple-600/80 text-white' : 'text-gray-500 hover:text-purple-400'
      }`}>
      {label}
    </button>
  );
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900/50 rounded-sm border border-[#1E293B] p-2 md:p-4 space-y-2 md:space-y-3 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-700">
      <h2 className="text-xs md:text-sm font-bold text-cyan-400 mb-1 md:mb-2 flex items-center gap-1 md:gap-2">
        <span className="text-purple-400 text-[10px] md:text-sm">🔮</span> CLAW ORACLE
      </h2>
        ...
        <>
          {categories.length > 0 && (
            <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)}
              className="w-full bg-black/30 border border-gray-700 rounded px-2 py-1.5 text-[9px] md:text-[10px] text-white outline-none focus:border-purple-500">
          ...
          <div className="space-y-2 md:space-y-2.5 min-h-0">
            {loading ? (
              <div className="border border-purple-800/40 bg-black/30 rounded p-4 text-center text-[9px] text-gray-500 animate-pulse">Loading rankings...</div>
            ) : filteredRankings.length === 0 ? (
              <div className="border border-gray-800/40 bg-black/30 rounded p-4 text-center text-[9px] text-gray-500">No data.</div>
              ...
              <button key={s.id} onClick={() => openReviews(s.id, s.name)}
                ...
                <div className="flex items-start gap-2">
                  <span className={`text-[10px] md:text-xs font-bold w-5 text-center shrink-0 ${i < 3 ? 'text-amber-400' : 'text-gray-600'}`}>#{i + 1}</span>
                  <div className="min-w-0 flex-1">
                    <h3 className="text-[10px] md:text-xs font-semibold text-white truncate">{s.name}</h3>
                    <p className="text-[8px] md:text-[9px] text-gray-400">{s.category} · {s.seller_name || 'Unknown'}</p>
                  </div>
                  <div className="text-right shrink-0 ml-1">
                    <div className={`text-[10px] md:text-xs font-bold ${ratingColor(s.avg_rating)}`}>{stars(s.avg_rating)}</div>
                    <div className="text-[7px] text-gray-500">{s.avg_rating.toFixed(1)} · {s.review_count} rev</div>
                  </div>
                </div>
              </button>
            ))}
          ...
          {categories.map(cat => (
            <div key={cat.category}
              ...
              <div className="flex justify-between items-center">
                <h3 className="text-[10px] md:text-xs font-semibold text-white">{cat.category}</h3>
                <span className={`text-[10px] md:text-xs font-bold ${ratingColor(cat.category_avg_rating)}`}>
              ...
              <div className="grid grid-cols-3 gap-1.5">
                {[['Skills', cat.skill_count, 'text-cyan-300'], ['Top-rated', cat.top_rated_count, 'text-amber-300'], ['Sold', cat.total_sales, 'text-green-300']].map(([k, v, c]) => (
                  <div key={k} className="space-y-0.5"><div className="text-[7px] text-gray-500">{k}</div><div className={`text-[9px] font-medium ${c}`}>{v}</div></div>
                ))}
              </div>
      ...
      {view === 'reviews' && (
        <div className="space-y-2 md:space-y-3 min-h-0">
          {selectedSkillId ? (
            <>
              <button onClick={() => setView('rankings')} className="text-[9px] md:text-[10px] text-cyan-400 hover:underline flex items-center gap-1">&larr; Back to Rankings</button>
              ...
              <div className="bg-black/30 rounded border border-l-2 border-l-purple-500 border-purple-800/30 p-2.5 md:p-3 space-y-1.5">
                ...
                <p className="text-[8px] text-gray-500">{reviews.length} reviews</p>
              </div>
              ...
              <div className="space-y-1.5">
                {reviews.length === 0 ? (
                  ...
                  <div key={r.review_id}
                    ...
                    <div className="flex justify-between items-center">
                      <span className={`text-[9px] md:text-[10px] font-medium ${ratingColor(r.rating)}`}>{stars(r.rating)}</span>
                      <div className="flex items-center gap-1">
                        <span className="text-[7px] text-gray-500">{r.reviewer_name?.slice(0, 8) || 'Anonymous'}</span>
                    ...
                    {r.comment && <p className="text-[8px] text-gray-300 leading-relaxed">{r.comment}</p>}
                    <div className="text-[6px] text-gray-600 mt-0.5">Weighted score: {r.weighted_rating.toFixed(2)}</div>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-1.5 pt-1 border-t border-slate-800">
                ...
                {needsAuth ? (
                  <div className="bg-black/30 rounded border border-l-2 border-l-purple-500 border-purple-800/30 p-3 space-y-1.5">
                    <p className="text-[8px] text-gray-400 text-center">Authenticate to post reviews</p>
                    <button onClick={() => window.dispatchEvent(new CustomEvent('xclaw:request-login'))}
                      className="w-full bg-purple-600 hover:bg-purple-700 text-white text-[9px] py-1.5 rounded font-medium transition-colors">Go to Auth</button>
                    <p className="text-[7px] text-gray-600 text-center">Use <code className="bg-slate-800 px-1 rounded">xclaw-skill login</code> to get your API Key</p>
                  </div>
                ) : (
                    ...
                    <div className="flex gap-1">
                      {[1, 2, 3, 4, 5].map(n => (
                        <button key={n} onClick={() => setReviewRating(n)}
                          className={`text-base transition-transform ${n <= reviewRating ? 'text-yellow-400 scale-110' : 'text-gray-600 hover:text-gray-400'}`}>&#9733;</button>
                      ))}
            ...
            <div className="border border-purple-800/40 bg-black/30 rounded p-4 text-center text-[9px] text-gray-500">
