...
export default function OsintStream() {
  ...
  return (
    <div className="border border-[#1E293B] bg-slate-900/50 backdrop-blur-sm rounded-sm p-2 md:p-4 flex flex-col">
      <h2 className="text-[10px] md:text-sm font-bold text-cyan-400 mb-1.5 md:mb-2">OSINT STREAM</h2>
      <div ref={scrollRef} className="space-y-1.5 md:space-y-2 flex-1 overflow-y-auto">
        <AnimatePresence>
          {logs.map(log => (
            ...
            >
              <span className="text-[10px] md:text-xs text-gray-500 whitespace-nowrap">{log.time}</span>
              <span className={`text-[10px] md:text-xs ${getLogTypeColor(log.type)}`}>{log.message}</span>
            </motion.div>
          ))}
        </AnimatePresence>
      </div>
