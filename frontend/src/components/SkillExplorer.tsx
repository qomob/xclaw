  ...
  if (collapsed) {
    return (
      <div className="flex flex-col items-center gap-2 pt-2">
        ...
        <span className="text-[7px] text-gray-400 text-center leading-tight">SKILL<br />EXPLR</span>
        {loading ? (
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900/50 rounded-sm border border-[#1E293B] p-2 md:p-4">
      <h2 className="text-xs md:text-sm font-bold text-cyan-400 mb-2 md:mb-4 flex items-center gap-1 md:gap-2">
        <span className="text-purple-400 text-[12px] md:text-sm">✧</span> SKILL EXPLORER
      </h2>
      ...
      <div className="flex gap-1 md:gap-2 mb-3 md:mb-4">
        <input 
          ...
          className="flex-1 bg-gray-800 border border-gray-700 rounded px-2 md:px-3 py-1 text-[12px] md:text-xs text-white focus:border-cyan-500 outline-none"
        />
        <button 
          onClick={() => fetchSkills(searchTerm)}
          className="bg-cyan-600 hover:bg-cyan-700 text-white text-[11px] md:text-[12px] px-2 md:px-3 py-1 rounded transition-colors whitespace-nowrap"
        >
          SEARCH
        </button>
      </div>
        ...
        {view === 'detail' ? (
          selectedSkill ? (
            <div className="space-y-3 md:space-y-4">
              <button
                onClick={handleBack}
                className="text-[12px] md:text-xs text-cyan-400 hover:text-cyan-300 transition-colors mb-2"
              >
                ← Back
              </button>
              <div className="p-3 md:p-4 bg-slate-800/50 border border-slate-700">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span className="text-[11px] md:text-[12px] bg-slate-700 px-1 md:px-1.5 py-0.5 rounded text-gray-400 uppercase">{selectedSkill.category}</span>
                </div>
                <p className="text-[11px] md:text-[12px] text-gray-400 mb-3">{selectedSkill.description}</p>
                ...
                {selectedSkill.tags && selectedSkill.tags.length > 0 && (
                  <div className="flex flex-wrap gap-0.5 md:gap-1 mt-3">
                    {selectedSkill.tags.map(tag => (
                      <span key={tag} className="text-[12px] md:text-[11px] text-cyan-500/70">#{tag}</span>
                    ))}
                  </div>
                )}
              </div>
            ...
            <div className="text-center py-8 md:py-10 text-[12px] md:text-xs text-gray-400">Skill not found.</div>
          )
        ) : loading ? (
          <div className="text-center py-8 md:py-10 text-[12px] md:text-xs text-gray-400 animate-pulse">Scanning network...</div>
        ) : skills.length === 0 ? (
          <div className="text-center py-8 md:py-10 text-[12px] md:text-xs text-gray-400">No matching skills found.</div>
        ) : (
          skills.map(skill => (
            <div key={skill.id} onClick={() => handleSelectSkill(skill.id)} className="cursor-pointer p-2 md:p-3 bg-slate-800/50 border border-slate-700 hover:border-cyan-500/50 transition-all group">
              <div className="flex justify-between items-start mb-1">
                <span className="text-[12px] md:text-xs font-bold text-white group-hover:text-cyan-400 transition-colors">{skill.name}</span>
                <span className="text-[11px] md:text-[12px] bg-slate-700 px-1 md:px-1.5 py-0.5 rounded text-gray-400 uppercase">{skill.category}</span>
              </div>
              <p className="text-[11px] md:text-[12px] text-gray-400 line-clamp-2 mb-1.5 md:mb-2">{skill.description}</p>
