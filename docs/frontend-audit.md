...
## 七轮重构（简洁现代布局 + 中英文双语，2026-08-06）

- **布局重构**：废弃「左侧 Sidebar + 顶部 Header + 底部 MobileNav」三套导航，改为单一顶部导航
  （TopHeader）：Logo + 主导航（网络/市场/任务/财务）+「更多」下拉（Agent/社交/协议/安全/管理）
  + 语言切换 + 单一登录入口；登录提示只保留在顶栏与移动端抽屉，消除重复 LOGIN。
- **移动端适配**：桌面完整导航；移动端汉堡按钮 → 侧滑抽屉（全部导航 + 语言切换 + 登录/退出）。
- **i18n 双语**：新增轻量 i18n 基础设施（LanguageContext + zh/en 字典，localStorage 持久化），
  覆盖导航/认证/首页 hero/实时面板/页脚/登录弹窗/九个页面标题；语言切换按钮常驻顶栏。
...
## 九轮（深层内容全量翻译 + 页脚精简，2026-08-06）

- **页脚精简**：删除「SYSTEM OPERATIONAL · API OK · DB UP · REDIS UP」状态行（页首已有状态点），
  页脚仅保留版权与链接。
- **深层翻译**：字典扩至 300+ 键，覆盖：
  - 任务中心全流程（我的任务/任务市场/创建、竞标、接标、提交结果、验收/驳回、状态文案、操作反馈）
  - 财务中心（总览/交易/多链钱包/充值说明）
  - 技能市场 ClawBay（发现/市场/热门/订单、精选、购买、运行技能、评价、认证面板、订单详情）
  - Agent 中心（搜索、任务统计、技能、社交关系、消息、记忆）
  - 社交图谱（信任分/推荐/社区、空状态）
  - 视图子标签（星系/拓扑/情报/图谱）与 协议/安全/管理 页签
- **验证**：构建通过；Playwright 实测 DATA 子标签双语（星系/拓扑/情报/图谱 ↔ GALAXY/TOPO/OSINT/GRAPH）；
  打包产物确认含全部新增中文文案。
